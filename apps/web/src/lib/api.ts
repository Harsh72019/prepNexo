...
async function request<T>(path: string, options: RequestOptions = {}) {
  ...
  if (response.status === 401 && options.accessToken) {
    ...
    if (refreshToken && path !== "/api/auth/refresh") {
      const refreshed = await rawRequest<ApiResponse<AuthSession>>(
        "/api/auth/refresh",
        {
          method: "POST",
      ...
      if (refreshed.ok) {
        ...
        const retried = await rawRequest<T>(path, {
          ...options,
  ...
  return fetch(`${options.baseUrl ?? env.authServiceUrl}${path}`, {
    ...
    .catch((error) => {
      if (error instanceof DOMException && error.name === "AbortError") {
        throw new Error(
          "Request timed out. Please check that the backend service is running.",
        );
      }
      throw error;
    })
...
export const authApi = {
  register: (body: { name: string; email: string; password: string }) =>
    request<ApiResponse<AuthSession>>("/api/auth/register", {
      method: "POST",
      body: JSON.stringify(body),
      timeoutMs: 20_000,
    }),
  login: (body: { email: string; password: string }) =>
    request<ApiResponse<AuthSession>>("/api/auth/login", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  refresh: (refreshToken: string) =>
    request<ApiResponse<AuthSession>>("/api/auth/refresh", {
      method: "POST",
      body: JSON.stringify({ refreshToken }),
    }),
  me: (accessToken: string) =>
    request<ApiResponse<AuthUser>>("/api/auth/me", { accessToken }),
...
export const dashboardApi = {
  summary: (accessToken: string) =>
    request<ApiResponse<DashboardSummary>>("/api/dashboard/summary", {
      accessToken,
    }),
};
...
export const billingApi = {
  ...
  status: (accessToken: string) =>
    request<ApiResponse<BillingStatusDto>>("/api/billing/status", {
      accessToken,
    }),
...
export const onboardingApi = {
  status: (accessToken: string) =>
    request<ApiResponse<OnboardingStatus>>("/api/onboarding/status", {
      accessToken,
    }),
...
export const practiceApi = {
  ...
  runCode: (body: {
    ...
    request<ApiResponse<{ ok: boolean; message: string }>>(
      "/api/interview/run",
      {
  ...
  adminQuestions: (accessToken: string) =>
...
export type QuestionAdminInput = {
  ...
  testCases: Array<{
    input: number[];
    expected: number | string | boolean | unknown[] | Record<string, unknown>;
  }>;
  examples: unknown[];
  constraints: unknown[];
  skillKeys: string[];
...
export const adaptiveApi = {
  growthProfile: (accessToken: string) =>
    request<ApiResponse<GrowthProfile>>("/api/adaptive/growth-profile", {
      accessToken,
    }),
  skillGraph: (accessToken: string) =>
    request<ApiResponse<SkillNodeDto[]>>("/api/adaptive/skill-graph", {
      accessToken,
    }),
  dailySession: (accessToken: string) =>
    request<ApiResponse<GrowthProfile["nextSession"]>>(
      "/api/adaptive/daily-session",
      { accessToken },
    ),
  pressure: (
    accessToken: string,
    body: { sessionId?: string; surface?: string; topic?: string },
  ) =>
...
export const arenaApi = {
  today: (accessToken: string, mode: "ranked" | "practice") =>
    request<ApiResponse<DailyArena>>(`/api/arena/today?mode=${mode}`, {
      accessToken,
    }),
  join: (accessToken: string, mode: "ranked" | "practice") =>
    request<ApiResponse<DailyArena>>("/api/arena/join", {
      accessToken,
      method: "POST",
  ...
  overall: (accessToken: string) =>
    request<ApiResponse<OverallLeaderboard>>("/api/arena/overall", {
      accessToken,
    }),
};
...
export const aiStreamApi = {
  interviewer: (
    body: { role: string; topic: string; message: string },
    onToken: (token: string) => void,
  ) => streamRequest("/api/ai/interviewer/stream", body, onToken),
  codeFeedback: (
    body: { language: string; code: string; prompt?: string },
    onToken: (token: string) => void,
  ) => streamRequest("/api/ai/code-feedback/stream", body, onToken),
  systemDesignFeedback: (
    body: { scenario: string; designNotes: string },
    onToken: (token: string) => void,
  ) => streamRequest("/api/ai/system-design-feedback/stream", body, onToken),
  roadmap: (
    body: { targetRole: string; weakTopics: string[]; recentScores: number[] },
    onToken: (token: string) => void,
