...
export class AuthService {
  async register(input: { name: string; email: string; password: string }) {
    ...
    if (existing) throw new HttpError(409, "An account already exists for this email", "EMAIL_IN_USE");
  ...
  async login(input: { email: string; password: string }) {
    ...
    if (!user?.passwordHash) throw new HttpError(401, "Invalid email or password", "INVALID_CREDENTIALS");
  ...
  async forgotPassword(email: string) {
    ...
    await sendEmail({
      ...
      html: `<p>Use this password reset link:</p><p><a href="${env.WEB_URL}/reset-password?token=${token}">Reset password</a></p>`
    });
  }

  async resetPassword(input: { token: string; password: string }) {
    const stored = await tokens.findAuthToken(sha256(input.token), TokenType.PASSWORD_RESET);
  ...
  async verifyEmail(token: string) {
    const stored = await tokens.findAuthToken(sha256(token), TokenType.EMAIL_VERIFICATION);
    if (!stored || stored.usedAt || stored.expiresAt < new Date()) {
      throw new HttpError(400, "Email verification token is invalid or expired", "INVALID_EMAIL_TOKEN");
    }
  ...
  async handleGoogleCallback(code: string) {
    ...
    if (!profile?.email || !profile.sub) throw new HttpError(401, "Google profile is missing required fields");
    const googleEmail = profile.email.toLowerCase();
    const googleName = profile.name ?? googleEmail.split("@")[0] ?? "Google Candidate";
    ...
    const user = await prisma.$transaction(async (tx) => {
      const account = await tx.oAuthAccount.findUnique({
        where: { provider_providerAccountId: { provider: "google", providerAccountId: googleSubject } },
        include: { user: true }
      });
      if (account) return account.user;

      const existing = await tx.user.findUnique({ where: { email: googleEmail } });
      const createdUser =
        existing ??
  ...
  private async createSession(user: Awaited<ReturnType<UserRepository["findById"]>>) {
  ...
  private async issueEmailVerification(userId: string, email: string) {
    ...
    await sendEmail({
      ...
      html: `<p>Confirm your email:</p><p><a href="${env.WEB_URL}/verify-email?token=${token}">Verify email</a></p>`
    });
  }
  ...
  private googleClient() {
    ...
    return new OAuth2Client(env.GOOGLE_CLIENT_ID, env.GOOGLE_CLIENT_SECRET, `${env.WEB_URL}/api/auth/google/callback`);
  }
}
